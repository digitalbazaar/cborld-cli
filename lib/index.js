...
async function encodeCommand(args) {
  try {
    const jsonldBytes = await _read(args);
    ...
    await _write(args, cborldBytes);

    if(args.verbose) {
      const ratio = 100 - (cborldBytes.length / jsonldBytes.length) * 100;
      console.log('[raw] ' +
        `${jsonldBytes.length} JSON-LD input bytes, ` +
        `${cborldBytes.length} CBOR-LD output bytes, ` +
        `${ratio.toFixed(0)}% smaller.`);

      const jsonldCBytes = canonicalize(jsonldDocument);
      const cratio = 100 - (cborldBytes.length / jsonldCBytes.length) * 100;
      console.log('[compact] ' +
        `${jsonldCBytes.length} JSON-LD input bytes, ` +
        `${cborldBytes.length} CBOR-LD output bytes, ` +
        `${cratio.toFixed(0)}% smaller.`);
...
// Yargs command to decode given CBOR-LD bytes to a JSON-LD document
async function decodeCommand(args) {
  try {
    const cborldBytes = await _read(args);
    ...
    await _write(args, jsonldBytes);

    if(args.verbose) {
      const ratio = ((jsonldBytes.length / cborldBytes.length) * 100) - 100;
      console.log('[indented] ' +
        `${cborldBytes.length} CBOR-LD input bytes, ` +
        `${jsonldBytes.length} JSON-LD output bytes, ` +
        `${ratio.toFixed(0)}% larger.`);

      const jsonldCBytes = JSON.stringify(jsonldDocument);
      const cratio = ((jsonldCBytes.length / cborldBytes.length) * 100) - 100;
      console.log('[compact] ' +
        `${cborldBytes.length} CBOR-LD input bytes, ` +
        `${jsonldCBytes.length} JSON-LD output bytes, ` +
        `${cratio.toFixed(0)}% larger.`);
...
// helper to write file or stdout
async function _write(args, data) {
  if(!args.output || args.output === '-') {
    _fs.writeSync(process.stdout.fd, data);
  } else {
    return fs.writeFile(args.output, data);
  }
}
...
async function documentLoader(docUrl) {
  ...
  try {
    ...
    const file = _fs.createWriteStream(cacheFile);
